...
@torch.library.custom_op("qwen3_dsa_approx::rotate_activation", mutates_args=())
...
class Qwen3DSAApproxServingIndexer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        assert capacity >= top_k
        assert capacity <= MAX_KERNEL_TOP_K, (
            f"index_topk={capacity} exceeds MAX_KERNEL_TOP_K={MAX_KERNEL_TOP_K}: vLLM 0.26.0's compiled "
    ...
    def forward(self, hidden_states: torch.Tensor, positions: torch.Tensor) -> torch.Tensor:
        ...
        # `RUNTIME.layer` is a plain-attribute scope, which Dynamo can trace. It used to set a
        # ContextVar, which it cannot (gb0156), and since vLLM 0.26 aot_compiles this forward in
        # fullgraph mode that made every ACTIVE selector fail at engine init. Still skipped when
        # there is nothing to attribute, so the `topk` control pays nothing.
