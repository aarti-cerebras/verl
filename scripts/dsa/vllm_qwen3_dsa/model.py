...
"""``Qwen3DSAForCausalLM`` — Qwen3-4B with token-granular DeepSeek Sparse Attention, on vLLM 0.26.0.
...
from .indexer import Qwen3DSAServingIndexer
...
def assert_servable_sparse(config: PretrainedConfig) -> None:
    ...
    assert int(index_topk) == top_k, (
        f"index_topk ({index_topk}) != dsa_top_k ({top_k}) -- the config describes two different "
        "selection budgets"
    )
...
class Qwen3DSAAttention(nn.Module):
    ...
    def __init__(
        ...
        self.indexer = Qwen3DSAServingIndexer(
            **dsa_params(config),
