...
register_backend(
    ...
    "scripts.dsa.vllm_qwen3_dsa_bucketed.sparse_attention.Qwen3DSASparseBackend",
)
...
class Qwen3DSASparseImpl(AttentionImpl):
    ...
    def forward(
        ...
        # Mandatory selector-to-FA3 contract checks. These stay active even when bucket telemetry
        # is disabled and synchronize only when CUDA reports a violation.
        valid = topk_indices >= 0
        torch._assert_async(
            ~((~valid[:, :-1]) & valid[:, 1:]).any(),
            "Qwen3 DSA bucketed indices are not valid-prefix/-1-suffix",
        )
        torch._assert_async(
            (valid.sum(-1).to(valid_counts.dtype) == valid_counts).all(),
            "Qwen3 DSA bucketed selector count differs from FA3 valid_counts",
        )
        ...
        torch._assert_async(
            ...
            "Qwen3 DSA bucketed selection maps to duplicate global KV slots",
        )
