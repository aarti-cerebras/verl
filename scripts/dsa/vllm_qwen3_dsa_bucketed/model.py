...
"""Isolated modulo-bucket local-top-k Qwen3 DSA serving on vLLM 0.26.0.

Snapshot source: ``scripts/dsa/vllm_qwen3_dsa``. The source revision is recorded by the derived
serving manifest, and the isolated bucket plugin never imports the exact or radix plugin.
...
from .bucket_selector_runtime import RUNTIME, config_from_hf
from .indexer import Qwen3DSABucketedServingIndexer
...
def assert_servable_sparse(config: PretrainedConfig) -> None:
    ...
    bucket_count = int(getattr(config, "dsa_bucket_count", 0))
    bucket_top_k = int(getattr(config, "dsa_bucket_top_k", 0))
...
class Qwen3DSAAttention(nn.Module):
    ...
    def __init__(
        ...
        self.indexer = Qwen3DSABucketedServingIndexer(
            **dsa_params(config),
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        if sparse:
            ...
            selector_config.validate_execution(
                cudagraph_mode=vllm_config.compilation_config.cudagraph_mode.name
            )
        ...
        if sparse:
            ...
            print(
                ...
                f"total_k={config.dsa_top_k} "
                f"(dense: {sorted(set(range(config.num_hidden_layers)) - sparse_ids) or 'none'})"
            )


class Qwen3DSABucketedForCausalLM(nn.Module, SupportsPP):
